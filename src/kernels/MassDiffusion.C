...
template<>
InputParameters validParams<MassDiffusion>()
{
...
Real MassDiffusion::computeQpResidual()
{
  // The (-1) is present as this is a source on the RHS.
  return _grad_u[_qp] * _grad_test[_i][_qp] * (_diffusivity[_qp]) * (exp(-0.1/_some_variable[_qp])) * (-1);
}

Real MassDiffusion::computeQpJacobian()
{
